explain select t.* from (
                    ...
                    from (
                        select m.* from access_log m
                        where
                          m.created_day_at between '2023-10-30' and '2023-10-31'
                          and (null is null or m.from_ip  = null)
                          and (null is null or m.from_port = null)
                          and (null is null or m.to_host = null)
                          and (null is null or m.to_port = null)
                          and ('top' is null or m.path <@ text2ltree('top'))
                          and (null is null or m.url like null || '%%')
                          and (null is null or m.url like null || '%%')
                          and (m.method in ('GET', 'POST', 'PUT'))
                          and (1=1 and (m.response_status_code not in (200)))
                          limit 10 offset 0
                    ) iol
